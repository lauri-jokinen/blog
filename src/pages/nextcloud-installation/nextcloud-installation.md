...
# Install Nextcloud
Let's go back to the server machine. Next we'll install Nextcloud snap-package. There are other ways, but this is by far the easiest.

You might want to look at manual install if
...
Manual installation is a bit of a hassle, and I recommend trying the snap package if you're a starter.

SSL should work with NC snap?
NC+RAID asetus?

FAQ:
New / other domain?
You can have multiple domains at the same time.
1. Register a new subdomain with same public IP.
2. Add the domain to trusted domains NC. (link)
3. Done!
